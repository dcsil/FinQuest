"""
Extended tests for users router to cover missing lines
"""
import pytest
from unittest.mock import Mock, MagicMock, patch, AsyncMock
from uuid import uuid4

from finquest_api.routers.users import get_suggestion_generator, generate_suggestions_task
from finquest_api.db.models import User


class TestUsersDependencies:
    """Tests for users router dependencies"""
    
    def test_get_suggestion_generator(self):
        """Test get_suggestion_generator dependency (lines 19-22)"""
    ...
    async def test_generate_suggestions_task_success(self):
        """Test generate_suggestions_task background task (lines 24-37)"""
        ...
        with patch('finquest_api.routers.users.SessionLocal', return_value=mock_db):
            with patch('finquest_api.routers.users.get_engine', return_value=Mock()):
    ...
    async def test_generate_suggestions_task_exception(self):
        ...
        with patch('finquest_api.routers.users.SessionLocal', return_value=mock_db):
            with patch('finquest_api.routers.users.get_engine', return_value=Mock()):
                # Should handle exception gracefully (prints error but doesn't raise)
                await generate_suggestions_task(mock_generator, str(mock_user_obj.id))
